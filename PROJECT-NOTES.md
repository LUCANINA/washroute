...
**⚠️ EDGE FUNCTIONS WITH `verify_jwt: true` NEED BOTH `apikey` AND `Authorization` HEADERS (session 129).** When the gateway sees `verify_jwt: true` on an edge function, it rejects the request with HTTP 401 *before* the function code runs unless `Authorization: Bearer <JWT>` is present. The `apikey` header alone is not enough. This bit us on the Route Command Center "Optimize" button — three call sites to `optimize-route` (`driver-app/index.html` `reoptimizeRoute`, `admin-dashboard/index.html` `autoOptimizeRoute`, `admin-dashboard/index.html` `optimizeRoute`) only sent `apikey` and all returned 401. Fix: always pair `apikey` with `Authorization: Bearer ${SUPA_KEY}` (or `SUPA_ANON_KEY`) for edge function calls. Today only two functions have `verify_jwt: true` — `optimize-route` and `get-stripe-fees` — and both are now called correctly. If you add a new edge function and leave `verify_jwt: true` (the Supabase default), every client call site must send the Authorization header. Consider a `callEdgeFn(slug, body)` helper to make it structurally impossible to omit.
