...
15. **`create-checkout` has no caller check** (session 293 QA) — uses the service-role key and trusts `customerId`/`userId` from the body. Still used by the subscription signup Checkout path (customer-app ~7481, admin ~33252). Lower risk than the card functions (it only opens a Stripe-hosted page), but add the same `authorizeForCustomer` guard used in `create-setup-intent`.
16. **Card-sheet onclick IDs not quote-escaped** (session 293 QA) — customer-app `openCardActions` / card option rows build `onclick="...('${escHtml(pmId)}')"`; `escHtml` doesn't escape `'`. Stripe `pm_` IDs are alphanumeric so no live risk; switch to `data-pm` attributes + delegated listener when next touched.
17. **Repo vs deployed card functions differ in comments only** (session 293) — `create-setup-intent` matches; `save-payment-method`, `remove-card`, `set-default-card` were deployed with a shortened auth comment header. Logic identical; redeploy from repo next time any is touched.
18. **Confirm-screen estimate omits sales tax** (pre-existing, noted session 293) — the estimate (and sticky CTA total) now shows discount + credit but no tax, so the charged amount can be a bit higher than shown.
