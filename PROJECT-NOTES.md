...
*Last updated: May 10, 2026 — Session 143. **Admin Map view: show all planned stops regardless of order processing status (Kidango fix).** Spotted via David: tomorrow's Kidango route showed 0 stops on the Map view even though 16 deliveries were on the books. Root cause was a hide-rule baked into the admin Map filters: delivery stops whose order was still in `picked_up` / `processing` / `folding` were silently filtered out, on the assumption "the bags aren't folded yet, don't show them to the driver." For Bay Area routes with a same-day pipeline that's correct, but Kidango (commercial route, multi-day turnaround, manual workflow) lives entirely at `picked_up` until delivery day — so all 16 stops vanished from the planning view.

**Pt 1 — Data fix.** Bulk-advanced the 16 Kidango orders for 5/11 from `picked_up` → `ready_for_delivery` so they appeared on the Map immediately. Snapshot at `_archive._kidango_advance_20260510` for reversibility. Preflight verified zero customer messages and zero charges: no `ready_for_delivery` SMS template exists, the charge trigger lives in admin app code (not in the DB so SQL UPDATEs don't fire it), and all 16 Kidango customers are `billing_type='on_account'` paid by check with no card on file. Triggers that fire on the UPDATE (`trg_log_order_change`, `trg_sync_stops_on_order_advance`) are audit/state-sync only — no fan-out.

**Pt 2 — Code fix (admin only) — show ALL planned stops on the Map view.** Loosened the hide-rule at six sites in `admin-dashboard/index.html`: the Map view chip-count loader (~19150), the Map view rccToggleRoute stop loader (~19326), the Map view rccRefreshRoute stop loader (~20377), the two Schedule grid today's-progress loaders (~18500, ~18717), and the driver-reassign popover liveStops loader (~18944). All six now count/show every alive stop regardless of the underlying order's processing status. Added a discrete amber **"🧺 awaiting wash / in wash / folding"** badge on delivery stop cards (rccBuildCard ~19630) so admin can distinguish planned-but-not-laundered stops at a glance — replaces the prior hard-hide behavior with a visible signal. Optimize-route gating still works because it sorts by lat/lng + time windows, not order status.

**Pt 3 — Driver app intentionally left as-is.** The driver app's identical NOT_READY_FOR_DELIVERY filter (driver-app line 1900) is correct behavior for that surface — drivers should only see actionable stops, otherwise they'd be sent to a location with no bag to deliver. Comment updated to explain why driver-side and admin-side now intentionally differ (driver = action surface, admin = planning surface).

**Architectural convention added — planning surfaces vs action surfaces.** The hide-rule was originally applied uniformly across all surfaces ("if the bag isn't ready, don't show it"). Session 143 pulls planning surfaces (admin Map, Schedule grid, reassign popover) out of that rule because the dispatcher needs the full picture to plan against — even if some bags aren't laundered yet. Action surfaces (driver app, and any future "click to mark delivered" surface) keep the rule. New planning views in admin should follow the same convention; new action views should keep the action filter.

**Tech-debt note for Kidango workflow.** The data fix advances 16 orders weekly. Long-term the cleanest fix is to either (a) auto-advance picked_up → ready_for_delivery on commercial-route templates after pickup completes, or (b) build a one-tap "mark route ready" button in admin for commercial routes. For now David can either run the same UPDATE manually each delivery day, or just leave them at picked_up — they'll be visible on the Map either way thanks to the code fix, with the amber "awaiting wash" badge clearly indicating the bags-not-yet-processed state.

---

*Prior: May 2, 2026 — Session 141. **Customer-facing coupon redemption + Linda vonHoene missing-order fix + Billing History tab.** Big session — six landings, all on top of a stale-deploy mystery that turned out to be a git author email problem.

**Pt 1 — Coupon redemption end-to-end (commits `dddcfb2`, migrations `session_141_discount_redemptions_table` + `session_141_redeem_discount_code_rpc`).** Customers can now enter promo codes themselves in the customer app, and admin can apply $ codes to customer accounts on the Billing tab. Single trusted door: `redeem_discount_code(p_code, p_customer_id, p_order_id, p_actor_name)` SECURITY DEFINER RPC handles validation, authorization (caller = customer-self OR `profiles.role='admin'`), and atomic write of redemption + credit/discount mutation. New `discount_redemptions` table is the source of truth for "who used what" — partial unique indexes enforce the type-specific reuse rules: $ codes single-use globally (`UNIQUE WHERE discount_type_snapshot='fixed'`), % codes one-per-customer (`UNIQUE (discount_id, customer_id) WHERE discount_type_snapshot='percent'`). Snapshot columns (`discount_type_snapshot`, `discount_value_snapshot`, `discount_name_snapshot`) preserve immutable history so admin edits to a discount don't rewrite the past. RLS: customers read their own, admin reads all, anon revoked, all writes through the RPC only. Behavior rules: % codes attach permanently to the customer account (updates `customers.discount_id`), highest-wins (refuses entry of an equal-or-lower % code with a friendly message); $ codes add the value to `customers.credits` + log a `credit_add` row in `customer_transactions` with description `Coupon X redeemed (admin:Name?)`. Customer-app: new "Promo Codes" section in Payment Settings (input + active % discount banner). Booking Confirm step: new sibling div `confirm-promo-section` (lives outside `confirm-summary` so summary rerenders don't clobber the typed input). Admin: new "APPLY PROMO CODE" section on customer Billing tab with a dropdown of unredeemed $ codes only — once applied, the code drops out of the list automatically. Existing Discount dropdown filtered to %-type only (cleaner UX; $ codes flow exclusively through the new path). DB-level RPC verified end-to-end with 11 test cases (empty, unauthenticated, %-happy-path, %-already-redeemed, lower-value rejection, %-upgrade, $-happy-path, $-single-use-globally, invalid, cross-customer-forbidden, admin-actor).

**Pt 2 — Linda vonHoene #3523 missing-order fix + root cause (commit `4453f03`, migration `session_141_pickup_failed_recovery_fix`).** John reported Linda's order wasn't showing up on his admin Routes view or Aracely's driver app even though it was scheduled for delivery this morning. Trace: order picked up by admin Luis manually after a driver `pickup_failed` cycle yesterday, proceeded normally through processing/folding/racking/`ready_for_delivery`, but the delivery `route_stops.status` stayed at `'skipped'` from yesterday — and both surfaces (driver app + admin Routes view) filter out skipped stops. **Root cause** was in `sync_stops_on_order_terminal` (the trigger that marks delivery stops `'skipped'` when an order goes `pickup_failed`): when the order subsequently recovered, no path restored those stops back to `'pending'`. The forward-advance trigger `sync_stops_on_order_status_advance` had a `WHERE status NOT IN ('complete','skipped','failed')` clause that explicitly blocked recovery. **Fix** — two-part: (a) modified `sync_stops_on_order_status_advance` to overwrite skipped/failed pickup stops as `complete` on forward advance, AND added a new branch that restores delivery stops from `skipped`/`failed` → `pending` when the order is back on the active flow before `out_for_delivery`. (b) extended `advance_order_status` RPC's `v_is_forward` definition to treat any move FROM a fail/skip/hold status TO an active status as a recovery, AND now clears stale `cancelled_by` + `driver_skip_reason` on forward advance (mirrors the existing `routing_error` clearing). Linda's stop also got a manual surgical fix: `status='pending'` + `driver_id` reassigned from Axel's drivers.id to Aracely's. Order #3523 became visible to Aracely within seconds of her pull-to-refresh.

**Pt 2 follow-up — historical zombie cleanup.** A blast-radius scan during the investigation found 20 zombie skipped-stops across 18 already-delivered orders going back to mid-March (10–50 row range — checked the pattern carefully per the widespread-issue rule before bulk-fixing). All matched the same Linda pattern: pickup_failed → admin recovered manually → order reached delivered via the alternate paths but the stop record was orphan-skipped from the original `pickup_failed` event. Snapshot `_archive._zombie_skipped_stops_20260502` (20 rows) captured the original state for reversibility, then bulk UPDATE to `complete` ran inside a single transaction. Verified all 18 orders still terminal `delivered` afterwards — the `trg_sync_order_status` AFTER UPDATE trigger fires on each row but its conditional only acts when current order status is `out_for_delivery`/`ready_for_delivery`/`scheduled`, so zero side-effects. Going forward the trigger fix from pt 2 prevents this class entirely; the audit skill could grow a Check that flags any active-flow order with a skipped delivery stop as a backstop.

**Pt 3 — Discounts list polish (commit `4a04a40`).** Two new right-aligned columns on the admin Discounts page: **Redeemed** and **Date**. Per row: $ codes show the dollar value of the single redemption + date (or "—"); % codes show "N customers" + most-recent-redemption date (or "—"). Same columns rendered on the Archived block (muted color). `loadDiscounts()` now does one round-trip with both `discounts` + `discount_redemptions` (Promise.all + group-by-id in JS, attached as `_redemptions` on each row). `_redemptionCells(d, opts)` keeps active + archived branches DRY.

**Pt 4 — Billing History tab in customer app (commit `673689d`).** New top-level Account menu item between Payment Settings and Laundry Preferences. Opens a single chronological view of every `customer_transactions` row for the customer: charges, credit additions (incl. coupon redemptions), credit uses, removals, refunds. RLS policy `ct_customer_read_own` already restricted to caller's own rows — no migration needed. `loadBillingHistory()` pulls from `customer_transactions`, side-fetches `order_number` via single batched IN query so rows render "Order #1234" instead of UUID. `_txMeta()` maps transaction type → icon + label + sign + color (charge/credit_use → "−" gray, credit_add/refund/credit_refund → "+" green). `renderBillingHistory()` groups rows by month, shows icon + label + date + order ref + description + card brand/last4 (when present) + amount. As part of this, the redemption-history list inside the Promo Codes section was REMOVED — replaced with a single line "Past codes appear in [Billing History →]" link — David's concern was that customers with many redemptions would clutter Payment Settings.

**Pt 5 — ⚠️ CRITICAL OPS LESSON: Vercel "Deployment Blocked" because of bad git author email.** Every commit pushed today (pts 1, 2, 3) sat in Vercel as 🔴 **Blocked** with the message: *"The deployment was blocked because the commit author email (`davidmacquart-moulin@Davids-MacBook-Air.local`) is not valid. Ensure your git email matches your GitHub account."* David's local git config had defaulted to the auto-generated `<username>@<hostname>.local` email instead of his actual GitHub email (`david@lucanina.com`). Production stayed on the previous `main` deploy (16h-old session-140-pt-4 docs commit) for hours while we thought everything was live. Discovered when David flagged "I don't see the new columns" → console check confirmed `_redemptionCells` was undefined on the served page → walked through the Vercel dashboard via computer-use → spotted the Blocked status. **Fix** — single combined command: `git config --global user.email "david@lucanina.com" && git config --global user.name "David Macquart-Moulin" && git rebase --exec "git commit --amend --no-edit --reset-author" HEAD~3 && git push --force-with-lease`. Rebase rewrote the 3 problem commits with the correct author; force-push replaced them on `main`; Vercel auto-deployed cleanly within ~30 seconds. **Going forward** all commits use the correct email so this can't recur — but if any future Mac is set up fresh, the same trap is waiting. Worth adding `git config --global user.email "david@lucanina.com"` to a "new-machine setup" checklist somewhere.

**Pt 5 minor — zsh nomatch gotcha.** During the rebase command, `rm -f .git/*.lock` (a defensive lock-cleanup that worked in earlier sessions) failed under zsh with "no matches found" because zsh's `nomatch` option errors out when a glob has no matches AND the chained `&&` blocks subsequent commands. Replaced with literal filenames (`/bin/rm -f .git/index.lock .git/HEAD.lock .git/ORIG_HEAD.lock 2>/dev/null;` — note the `;` not `&&` so the chain continues even if rm has nothing to delete). Worth using this pattern in any future paste-into-terminal scripts that defensively clear lock files.

**Architectural state at close.** Coupon system is now self-serve for customers, admin-applied for $ codes (via the same RPC — single trusted door with caller authorization). Discount redemptions are immutable + unique-constrained at the index level, so the categorical promises hold even if app code grows new entry points. The pickup_failed recovery hole is closed both at the trigger layer (auto-restore on forward advance) and the RPC layer (clear stale cancelled_by + driver_skip_reason on recovery). Customer billing surface is now centralized — Payment Settings is for inputs (card + tip + promo), Billing History is for the read-only ledger.

**Tech debt left.**
- Driver-app/admin-Routes filter logic that hides skipped stops should still be there — it's correct for normal skip semantics. The trigger fix prevents the zombie class going forward; the historical cleanup healed the past. No code change needed there.
- Audit skill could grow a Check that flags any order in `picked_up`/`processing`/`folding`/`ready_for_delivery` with a skipped/failed route_stop. Tiny, low priority — would catch any regression of the pt 2 trigger fix.
- The `loadPromoCodeData()` helper still fetches `discount_redemptions` for the customer even though the history list was moved to Billing History. Harmless extra query; can be slimmed when convenient (just keep the active-discount-id fetch).
- Customer-app `acct-menu-billing-info` mi-value div is rendered empty. Could populate with a count or current credits balance for at-a-glance.

**Pt 6 — Legacy pricelist CHECK constraints dropped (migration `session_141_drop_legacy_pricelist_check`).** David created a new price list "HCEB" via the admin Price Lists tab, then tried to add a service under it and hit `new row for relation "services" violates check constraint "services_pricelist_check"`. Trace: the `services_pricelist_check` and `customers_pricelist_check` constraints hardcoded the enum (`Delivery`/`Commercial`/`Retail` for services, `Delivery`/`Commercial` for customers) from before the `service_categories` table existed. `service_categories` is now the actual source of truth — admin UI populates pricelist dropdowns from it — but the two constraints never got updated when categories went user-editable. Dropped both. **Bonus catch:** `customers_pricelist_check` was missing `'Retail'` anyway (would have silently rejected any future customer pricelist=Retail). **Tech debt note:** cleaner long-term design is to make `services.pricelist` + `customers.pricelist` proper FKs to `service_categories.id`, so renaming a price list ripples instead of orphaning rows. Out of scope for today; flagged for a future session.

**Pt 7 — Customer Price List dropdowns populate from `service_categories` (commit `3250936`).** Same hardcoded-enum bug as pt 6 but on the UI side: `cpe-type` (Customer Panel → Contact tab) and `c-pricelist` (Add Customer modal) had `<option>` literals for Delivery/Commercial only. Stripped both to empty selects, populated dynamically from `allCategories` filtered to non-Retail (Retail is POS-only — same convention as Services tab). Defensive `if (!allCategories.length) await loadCategories()` at both call sites so the dropdowns work even before the user visits the Services tab. If a customer's existing pricelist isn't in the active categories (renamed/deleted), it appears as a sticky `'X (legacy)'` option so saving doesn't silently overwrite their data. With pt 6 + pt 7, admin can now move a customer to HCEB end-to-end.

**Pt 8 — Generalise pricelist→service resolver beyond Commercial (admin + customer-app).** With Extended Stay, Homebase, and Soul Sanctuary moved to HCEB, an intake on any of their active orders would have silently fallen back to the Delivery service ($1.75 Wash & Fold) instead of HCEB Wash & Dry ($1.70). Three code paths were hardcoded `customerPricelist === 'Commercial' ? Commercial : Delivery`. Generalised the pattern: any non-Delivery pricelist looks up its own service in the cache; Delivery default is the fallback. Sites: admin `openIntakePanel` (intake calc procSvc resolver, line ~23104), admin `onOrderCustomerChange` (saveOrder modal auto-select, line ~14814), customer-app `loadServices` + `getCustomerService` (replaced single `_commercialService` global with `_pricelistServices` map keyed by pricelist name; lazy-loaded from services where pricelist <> 'Delivery'). Same bug class as pt 6/7 — generalisation closes the door on it across the codebase.

**Pt 8 follow-up — HCEB April reprice (33 orders, $9,860.60 → $9,567.85).** All April 2026 delivered orders for the three HCEB accounts were billed at the old Commercial $1.75/lb rate (since they were on Commercial then). All on-account, none charged yet (`billing_status=NULL`). Snapshot to `_archive._hceb_reprice_20260502`, then transactional UPDATE rebuilt the base line_item at $1.70/lb, preserved delivery fee + any other items, recomputed `total_amount`. Followed up with a label refinement at David's request: base label now reads `Wash & Dry · X bags · Y lbs × $1.70/lb` (singular/plural handled). David sends invoices via admin Bill Orders flow.

**Pt 9 — Enrich line_item base label with service name + bag count (code + historical) (DB pt + 5 code sites).** David noticed Commercial customer invoices had `"X lbs × $1.75/lb"` with no service name or bag count. Same omission on per_bag (no service name). Updated 5 code sites that build base labels (admin opRecalcEstimate, opUpdateCostBreakdown, onOrderCustomerChange/Add Order modal, _buildIntakeLineItems, customer-app placeOrder) to use the canonical format: per_lb → `'ServiceName · N bag(s) · weight lbs × $rate/lb'`, per_bag → `'ServiceName · N bag(s) × $rate'`. Bag clause omitted on per_lb when bags=0; service prefix omitted when unresolved. Historical backfill of all eligible Commercial orders (108 rows = 74 per_lb + 34 per_bag) snapshotted to `_archive._commercial_relabel_20260502` then in-place updated with `jsonb_set` on `line_items->0->>'label'`.

**Pt 9 follow-up — Reup Refill Shop $136.50 refund batch.** Discovered while preparing the relabel that 34 Commercial-customer orders had service_id pointing at Wash & Fold (Delivery, $59/bag) instead of Commercial ($1.75/lb) — pre-pt-8 architecture bug. Most customers (Anita Hodzic et al) had already been manually refunded by David in prior sessions; only Reup Refill Shop's 7 affected orders remained outstanding. Per-order overcharge calculated against Commercial $1.75/lb pricing accounting for credit-applied complications on orders #747 and #1259. Six Stripe refunds issued via `refund-charge` edge function — added a new `suppress_sms: true` flag to the function so we could batch-refund silently, then David sent one consolidated explanatory SMS to Reup. Total: $136.50 refunded across 6 orders. Order #3363 (the remaining unpaid scheduled one) was re-pointed to Wash & Fold (Commercial) with `line_items` cleared so the next intake recomputes fresh. `refund-charge` v16 deployed.

**Pt 10 + Pt 11 — Invoice modal remembers To/CC/Subject/Body across sends (commits `8938c67` + group-side update).** Sending invoices via the admin Bill Orders → Send Invoice modal lost any edits — next time, the same template defaults reappeared. Now persisted both for billing-group invoices (saves back to `billing_groups.contacts` + `email_subject_template` + `email_body_template`) and for standalone individual-customer invoices (new migration `session_141_customer_invoice_template_columns` adds 4 nullable text columns to `customers`: `invoice_to_email`, `invoice_cc_emails`, `invoice_subject_template`, `invoice_body_template`). Save-back is fire-and-forget — if the persistence write fails the email send already succeeded; we just log a warning. **Smart period unwrap**: if the rendered period string ('April 2026') appears in the saved subject/body, it's converted back to `{{period}}` so next month's render auto-updates without editing.

**Pt 12 — Tech-debt sweep (commit `8dc7886`).** Three small cleanups closing items the prior parts flagged: (a) customer-app `loadPromoCodeData()` no longer fetches `discount_redemptions` — after Billing History moved off Payment Settings (pt 4), nothing reads the redemptions cache anymore. Removed the cache global + the query. (b) Account menu "Billing History" sub-label now shows `'N transactions · last MMM D'` (or `'No activity yet'`) from a cheap HEAD-count + 1-row lookup at account-page open. Was rendering empty before. (c) `washroute-audit` skill: new **Check 21** — flag any active-flow order (`picked_up`/`processing`/`folding`/`ready_for_delivery`) with a `skipped`/`failed` route_stop. Backstop for the pickup_failed recovery trigger fix from pt 2; if any rows ever surface, the trigger has regressed or a new code path bypassed it. Skill repacked + ready for reinstall.

**Pt 13 — Gift Up! webhook integration (edge function `giftup-webhook` v9).** Receives `giftcardcreated` / `giftcardupdated` / `giftcardrefunded` / `giftcardvoided` events from Gift Up! and turns them into rows in `discounts` so the recipient can enter the code in the customer app and have it land as account credit (existing `redeem_discount_code` RPC takes it from there). **Verified Gift Up specifics from live testing:**
- Header: `x-request-signature-sha-256: t=<unix_ms>,sha256=<hex_hmac>`
- Signed payload: **`raw_body + t`** (body concatenated with timestamp at the end — NOT Stripe's `t.body` format)
- Secret: used as raw string (not base64-decoded)
- Event names: one word, no dot (`giftcardcreated` etc) — the function's `parsePayload` strips dots/underscores so both formats resolve
- Idempotent: insert by `discounts.name` is a no-op when the code already exists (handles Gift Up's automatic retries)
- Refund/void/`status=refunded` updates: archive the corresponding discount so it can no longer be redeemed

Deployment took 6 iterations (v1 → v9) to nail the signature format from logs alone — Gift Up's docs don't pin the format precisely. Final v9 keeps only the verified `body + t` string-secret formula. Env vars required: `GIFTUP_WEBHOOK_SECRET` (in Supabase Secrets, must match the Secret field on each Gift Up webhook), `GIFTUP_API_KEY` (saved for future Redeem-callback use). Webhook URL configured in Gift Up: `https://umjpbuxrdydwejqtensq.supabase.co/functions/v1/giftup-webhook` (one webhook per event subscription, both share the same secret + URL).

**Pt 13 follow-up tech debt:** outbound Redeem call — when a customer redeems a Gift Up code in WashRoute, we should also call Gift Up's `Gift Cards - Redeem` endpoint to mark the card used in their system. Without this, Gift Up's outstanding-balance reports stay inflated. Pure bookkeeping; doesn't affect customers. Worth doing as a v10 of the function.

**Architectural state at session close.** Coupon system is end-to-end self-serve for customers (entry in payment settings + booking confirm), admin-applied for $ codes (via the same RPC — single trusted door with caller authorization), AND fed automatically from Gift Up! gift card sales (webhook → discount). All three paths converge on the same `discount_redemptions` ledger and `redeem_discount_code` RPC. Pickup_failed recovery hole is closed at the trigger layer, RPC layer, and audit layer. Customer billing surface is centralized — Payment Settings is for inputs (card + tip + promo), Billing History is the read-only ledger. Pricelist resolution is generalised across admin + customer-app — adding new price lists no longer requires code changes. Invoice modal remembers everything per-customer or per-group with smart period auto-unwrap. **18 commits and 5 migrations + 1 edge function shipped.**

**Migrations applied this session:** `session_141_discount_redemptions_table`, `session_141_redeem_discount_code_rpc`, `session_141_pickup_failed_recovery_fix`, `session_141_drop_legacy_pricelist_check`, `session_141_customer_invoice_template_columns`. **Edge functions:** `refund-charge` v16 (suppress_sms flag), `giftup-webhook` v1→v9.*

*Prior: May 1, 2026 — Session 140 (pt 4). **Driver-app addresses bypass the RLS chain — Jose Ramirez fix.** New driver Jose Ramirez (zepeda445@gmail.com) reported that customer addresses showed as "Sin dirección" on every stop in his driver app, making Google Maps navigation impossible. His daughter then logged in as him on a fresh iPhone — same result. So the bug followed his account, not his device.

**Diagnostic dead-ends:** Jose's `auth.users` row was identical to working drivers (role=authenticated, email confirmed, not banned). His `profiles` row had `role='driver'`. His `drivers` row was correctly linked via `profile_id`. His 44 pending stops all had valid `address_id` values pointing to addresses with proper coords + matching customer_ids. Every RLS policy in the chain (profiles, drivers, route_stops, orders, addresses) looked correct. **SQL simulation as Jose** (`SET LOCAL request.jwt.claim.sub`) returned the addresses correctly via the existing `driver_read_stop_addresses` RLS policy. But his real Postgrest requests came back empty for reasons we could not reproduce in the SQL editor.

**Fix per the project's RPC-pattern principle (sessions 134/135):** stop relying on the convoluted addresses RLS chain for driver address reads.

- **Migration `session_140_get_driver_stop_addresses_rpc`:** new SECURITY DEFINER function `get_driver_stop_addresses(p_stop_ids uuid[])` returns one row per stop with the resolved address (`rs.address_id` primary; `orders.pickup_address_id` / `orders.delivery_address_id` fallback for stops with null `address_id`). Authorization enforced inside the function — caller must match `rs.driver_id` OR `r.driver_id` OR `r.pickup_driver_id` OR `r.delivery_driver_id`. `anon` explicitly REVOKEd; `authenticated` granted EXECUTE. Verified: returns 24 rows for Jose's pending stops vs 0 from the prior path.
- **Driver-app (`driver-app/index.html`):** `loadDriverData()` now calls the RPC via `db.rpc('get_driver_stop_addresses', { p_stop_ids: stopIds })` instead of the prior two-query side-fetch + graft pattern. Each stop gets `_addr` populated keyed by `stop_id`. Net: -12 lines, single query, immune to the RLS edge case.
- **Verified working** — Jose pulled-to-refresh after the Vercel deploy and addresses populated correctly across his stops.

⚠️ **Tech-debt note:** the underlying root cause was never identified. Jose's auth context evaluates the addresses RLS to empty in production while the same SQL evaluates correctly in simulation. If we observe this again with another driver, the RPC keeps them functional, but the policy mystery itself remains open.

---

*Prior: May 1, 2026 — Session 140 (pt 3). **Sales tax v2 — per-row taxable flag replaces the type-level rule; checkbox columns added across Services, Merchandise, Fees; delivery orders now uniform with POS.** David asked for explicit per-row control rather than the type-level rule shipped in pt 2 ("merchandise yes, services no"). The pt 2 architecture had `isLineTaxable(line)` in the POS infer taxability from line.type/service; pt 3 replaces that with a literal read of a new `taxable` boolean column on each catalog row. Admin can now toggle tax on or off for any single service, merchandise item, or fee — most flexible model.

- **Migration `session_140_per_row_taxable`:** added `BOOLEAN NOT NULL DEFAULT FALSE` to `services`, `merchandise`, `service_fees`. Existing 36 merchandise rows backfilled to `TRUE` so day-1 behaviour matches the pt 2 type-level rule (no surprises). Existing services + fees default `FALSE` (admin opts in). Reversible with three `DROP COLUMN`s.
- **POS (`pos/index.html`):** `dbRowToTile()` and the merchandise tile-build now read `taxable` from each DB row and stamp it on the cart tile. `isLineTaxable(line)` simplified to `line.taxable === true`. `cartTaxableSubtotal()` upgraded to walk parent + addons independently — a per_lb laundry service can be taxable or not, and each addon under it carries its own flag, so mixed configurations work correctly. Each line_item written to `orders.line_items` carries the same `taxable: bool` snapshot.
- **Customer-app (`customer-app/index.html`):** new `BIZ_TAX_RATE` global, loaded from `settings.sales_tax_rate` inside `loadServices()`. `service_fees` SELECT now pulls `taxable`. `placeOrder()` stamps `taxable` on each line_item it builds (base service from `getCustomerService().taxable`; preferences/addons currently always false — `preferences` table has no taxable column yet; delivery_fee taxable iff EVERY underlying routeFee is taxable; same_day_surcharge taxable from the matching `service_fees` row). Computes `tax_amount` from the taxable subtotal; `total_amount` now equals `subtotal + taxAmount` (post-tax, pre-tip) — matches POS semantic so charge-order keeps charging `total_amount + tip_amount` correctly. Persists `tax_amount` to the new column. **Effect today:** zero — no services or fees are flagged taxable yet. **Effect when David ticks a box:** delivery laundry orders that include taxable items will compute and charge tax automatically.
- **Admin (`admin-dashboard/index.html`):** added "Tax" toggle column to four UI surfaces:
  - **S&P → Services tab** — every laundry service row. New `toggleServiceTaxable(id, taxable)` save handler. CSS grid widened from 5 to 6 columns.
  - **S&P → Point of Sale → Laundry Services** — same shape; `pos-retail-row` grid widened. `updateRetailSvc(id, 'taxable', value)` extended.
  - **S&P → Point of Sale → Merchandise** — `merch-row` grid widened. `updateMerch(id, 'taxable', value)` extended.
  - **S&P → Fees** — `fee-row` grid widened to 7 columns. New `toggleFeeTaxable(id, taxable)` save handler.
  Each toggle auto-saves on click + shows a green-checkmark "Tax: on/off" toast. The pt 2 global Sales Tax Rate input at the top of the Fees tab still controls the rate itself.
- **Admin Add Order modal (`saveOrder()`):** stamps taxable on each line_item it constructs. Computes `tax_amount` from the taxable portion and writes it to the order. Preserves the pre-existing semantic where this path's `total_amount` already lumps tip in (a quirk separate from POS/customer-app — flagged as tech debt but out of scope here).

**QA blast-radius pass:** `isLineTaxable` only referenced in pos/index.html (1 place — clean). `taxable` field on line_items only consumed in tax-math sites (cleanly stamped at every write site). Customer-app rename of `total → subtotal` checked — `total` is redeclared after taxAmount is computed; receipt-email body uses the new post-tax total correctly.

**Tech debt left (small):**
- `preferences` table has no `taxable` column. Customer-app addons (Vinegar/Oxi/etc.) hardcode `taxable: false`. Add the column + per-row UI if a preference ever needs to be taxable.
- Customer-app rolls all routeFees into one `delivery_fee` line_item; the line is taxable only when EVERY underlying routeFee is. Splitting per-fee into separate line_items would handle mixed configurations precisely. Not relevant today (single delivery fee row).
- Admin Add Order modal's `total_amount` includes tip (lumped with subtotal + fees + surcharge + tax + tip). POS + customer-app keep tip separate. Worth reconciling — admin path is rare so impact is small.
- Admin saveIntake (POS intake form) and `opSaveDetails` (admin edit modal) rebuild `line_items` mid-flow but don't yet stamp `taxable` on the rebuilt items. If a POS order is moved back to Intake, its line items lose their taxable bits. To follow up.

Migrations applied: `session_140_per_row_taxable`. Code commits this part: pending.*

*Prior: May 1, 2026 — Session 140 (pt 2). **Foothill printer hardware swap + cloudprnt multi-printer routing fix + POS PWA chrome + sales tax (the big one).** Big in-store afternoon at Foothill. Three independent landings.

**Pt 2a — New Star mC-Print3 at Foothill register, with a cloudprnt edge-function bug caught in the process.** Walked David through unboxing, WiFi-onboarding, Browse-to-IP CloudPRNT settings, all the standard mC-Print3 stuff. New token `00:11:62:36:30:7C` swapped onto `pos_devices` (Foothill Register row) replacing the old `00:11:62:35:E8:93`. **Bug found mid-test:** queued a test print, status went `pending → claimed` immediately… but no paper at Foothill. Edge-function logs showed the JOB had been served — to a printer with MAC `00:11:62:35:E8:93`. That MAC is set on `settings.printer_token` (the main store's mC-Print3) AND was also the OLD Foothill token. Investigation revealed `cloudprnt/index.ts` was selecting `print_jobs` rows with NO `printer_token` filter — just "oldest pending job" — so any printer that polled first won the race regardless of who the job was for. Fine when the system had one printer; broken the second a second printer joined. **Fix (cloudprnt v18, commit `95ee3f5`):** every handler branch (poll, serve-binary, complete-via-DELETE, POST poll) now adds `.ilike('printer_token', mac)` against the `?mac=...` query param the printer always sends. Case-insensitive (printers send lowercase, DB stores uppercase). Backward-compat fallback when `mac` is missing (legacy/test callers). Test print re-queued, printed correctly at Foothill on the new register, main-store printer stayed idle. **Latent data quality issue surfaced:** `pos_devices.printer_token` had previously been copied from `settings.printer_token` rather than reflecting Foothill's actual hardware — so even with the function fix, the Foothill register would have routed back to the main store under the old token. Fixed by the swap.

**Pt 2b — POS PWA chrome (commit `9347d19`).** Pre-pt-2b, `pos/index.html` had no PWA meta tags at all — `<title>` was placeholder "Family Laundry POS — Mockup" and "Add to Home Screen" picked up a Safari screenshot for the icon. Added the full PWA chrome the other three apps already had: title → "Family Laundry POS"; `apple-mobile-web-app-title`, `apple-mobile-web-app-capable`, `apple-mobile-web-app-status-bar-style: black-translucent`, `theme-color: #14b8a6`; `apple-touch-icon` (180px) + `icon` (512px). **New icon: teal-gradient (`#14B8A6 → #0D6E64`) variant of the canonical white "Family Laundry / Oakland CA" wordmark.** Built via a small Python+PIL script (`build_pos_icon.py`, kept in outputs/) that extracts the wordmark mask from `assets/icon-customer.png` and composites it onto a fresh diagonal gradient — guarantees pixel-identical wordmark across all four app icons (customer/admin/driver/POS), only the background color differs. Distinct from customer pink, admin navy, driver mustard so the four icons read as a set on the iPad home screen. Files: `assets/icon-pos.png`, `assets/icon-pos-180.png`. **Operationally:** David added it to the iPad home screen after the Vercel redeploy; verified that "delete + re-add" is the right ritual to refresh both the title and icon (Safari caches both per-bookmark).

**Pt 2c — Sales tax, end-to-end (commit `3c0180c`).** Discovered while planning that a previous session had wired up partial tax math (hardcoded TAX_RATE = 0.0925 in pos/index.html, taxed everything including services). Wrong rate, wrong rule. Built proper tax architecture in one pass.

- **Migration `session_140_sales_tax_columns`:** `settings.sales_tax_rate NUMERIC(6,4) NOT NULL DEFAULT 0.1075` (Oakland CA 10.75%) + `orders.tax_amount NUMERIC(8,2) NOT NULL DEFAULT 0` + `CHECK (tax_amount >= 0)`. All 3,156 existing orders default to `tax_amount = 0` (correct — no retroactive tax). Reversible with a clean `DROP COLUMN` pair.
- **Type-level rule (CA-correct):** laundry services exempt, merchandise taxable. New helper `isLineTaxable(line)` in pos/index.html — `per_lb` services and Retail-tab service tiles return false; merchandise (drinks/bags/snacks/supplies) returns true. New `cartTaxableSubtotal()` helper used at every cart-math site (`updateTotals`, `openPay`, `payWithCash`, terminal-modal compute, `createPosOrder`). **QA blast-radius pass found two stale sites** (`openPay` line 4709, `payWithCash` line 4730) that were still passing the FULL subtotal to `_taxOn()` — meaning services were being taxed in the cash + pay-method-picker payment paths even after the cart-display fix. Both fixed before commit. All 5 `_taxOn(` call sites in the live POS now pre-filter taxable lines.
- **Settings-driven:** `let TAX_RATE = 0.1075` (default) replaced the prior `const`, then a `loadTaxRate()` helper fetches `settings.sales_tax_rate` at app boot alongside `loadRetailServices()`. Admin can change the rate without a code deploy.
- **Each line item carries a `taxable: bool` snapshot** (POS `createPosOrder`) so historical orders survive any future rule change. `total_amount` semantic preserved (still = subtotal + tax, pre-tip) so `orderChargedTotal()` and Stripe Payment Intent math need no changes. Synthetic `type='tax'` line_item dropped — `tax_amount` column is now the single source of truth.
- **Receipt rendering** (3 surfaces): POS Star Markup, POS HTML fallback, send-receipt edge function v25 — all now show a `Subtotal / Tax (10.75%) / Tip / TOTAL` block. Each pulls tax from the new column AND falls back to the legacy `type='tax'` line_item (so orders created before this commit still render correctly). send-receipt redeployed.
- **Admin Services & Pricing → Fees:** new "Sales Tax Rate" input pinned at the top of the Fees tab. Auto-saves on edit (1.5s debounce), green-dot confirmation, range-validated 0–50%. `BIZ_TAX_RATE` global cached from `settings` at startup.
- **Admin Reports → Revenue:** new "Sales Tax Collected" green pill above the daily breakdown (hidden when `tot.tax === 0`, e.g. delivery-only date ranges). `tax` added to the per-day row aggregates and the totals reduce. CSV export gains a "Sales Tax Collected" column for tax-filing reconciliation.

**Pt 2d — small infrastructure note:** sandbox can't unlink git lock files — every commit attempt left a stale `.git/index.lock` or `.git/HEAD.lock` that David had to clear from his terminal between commits. Workaround for the rest of the session was to bundle `rm -f .git/*.lock && git add ... && git commit -m ... && git push` into a single paste-into-Terminal block. Worth fixing if it keeps happening — could be a Cowork mode quirk on macOS bind mounts.

**Tech debt left (small):**
- send-receipt's "Sales tax (10.75%) $X.XX" label only renders the percentage when the legacy `type='tax'` line_item is present (it has `rate` baked in). New column-only orders render "Sales tax $X.XX" without the percent. Could query settings inside the edge function to fix — minor polish.
- `pos-mockup.html` (the stale Apr-24 mockup) still has hardcoded `TAX_RATE = 0.0925` in 5 sites. Not referenced by any deployed code, but the file is still in the repo. Worth deleting.
- Per-lb pricing math (from pt 1) still duplicated across 7 sites — `getOrderPricing()` extraction would make pt 1's bug class structurally impossible.

Commits this session: `95ee3f5` (cloudprnt MAC filter), `9347d19` (POS PWA chrome), `3c0180c` (sales tax). All pushed; Vercel redeploy auto-fired for admin + POS. send-receipt edge function v25 + cloudprnt edge function v18 deployed.*

*Prior: May 1, 2026 — Session 140 (pt 1). **Per-lb pricing fix on customer-app Confirm screen + admin Add Order modal.** While placing a test order on the Commercial pricelist, David noticed the Bag Count step correctly showed `$53.70` (25 lbs × $1.75 + $9.95) but the Confirm screen showed `$16.70` ($1.75 × 1 bag + $9.95 + $5 tip). Root cause: `buildConfirmSummary()` in `customer-app/index.html` and `saveOrder()` in `admin-dashboard/index.html` (the manual "Add Order" modal) were both computing `base_price * bags` regardless of the service's `pricing_type`. Every other pricing site in the codebase already correctly branches on `isPerLb` (`updateEstimate`, `placeOrder`, `saveEditOrder`, admin `opSaveDetails`, admin `opUpdateCostBreakdown`) — these two were the lone holdouts. Fixed both to mirror the canonical per-lb math. The customer-side fix corrects the visible total and the breakdown line ("Wash & Fold · Est. 25 lbs × $1.75/lb · $43.75"); the admin-side fix corrects `total_amount` and `line_items[base].label` saved on Add-Order-modal-created orders. Driver app doesn't touch pricing — clean. Customer app's `placeOrder` (the actual order INSERT) was already correct, so no Commercial customer was actually billed wrong via the customer app — they would just have seen a misleadingly low estimate on the Confirm screen and then placed an order that saved the correct higher amount. The admin Add Order modal would have actually saved the wrong total, so any Commercial order that went through it should be re-checked. **Tech debt logged:** the per-lb math is now duplicated across 7 sites — extracting it into a single `getOrderPricing()` helper would make this class of bug structurally impossible. Also `services.pricing_type` should be `NOT NULL DEFAULT 'per_bag'` so a service can never be saved with a null type. Commit `3f271d9`.*

*Prior: Apr 28, 2026 — Session 137 (pt 8 — full POS theming pass closed). **POS theme migrated end-to-end from purple to admin blue + brand navy + green-as-status-only.** 11 incremental commits worked through every dark/colored surface in the POS file. **Color identity at close:** `--accent` (#3b82f6 / #1d4ed8 / #dbeafe) for all interactive chrome and primary actions (charge button, customer-chip-attached, queue active, hover states everywhere); `--brand-navy` (#0f2744, matching admin's `--sidebar-bg`) for primary dark surfaces (active tab pill, walk-in CTA, payment-total bar, flow-button active); semantic green/amber/red reserved exclusively for status (reader connected, billing failed, etc.). **One `--info-blue` alias family remains** as a back-compat layer — same hex as `--accent`, can be collapsed in a future cleanup pass. **Topbar contents at close:** 76px tall, brand wordmark on left (logo_white_bg.png, 48px tall, with right-border divider), then cashier pill (avatar + name + ▾, click to switch), customer chip (renamed default "Walk-in"→"Customer"), site selector (auto-hides when only one active site), refund pill, queue badge, reader-status pill (chromeless when connected — just dot + name). **Removed entirely:** standalone "Switch" cashier button (folded into cashier pill), in-topbar clock (OS chrome shows it), verbose two-element reader-status block. **All chrome elements unified at 40px height.** **QA + security review:** all DOM ID wiring intact, asset path resolves, no innerHTML writes added (one removed), no credentials introduced, all text injections via .textContent, JS parses clean. **Outstanding UX call:** `.btn-success` (cash-complete-sale modal) still green — David to decide if it should also go blue. **Earlier in session 137:** PICKUP keyword fixed.

*Apr 29, 2026 — Session 139. **Voicemail receive pipeline shipped + Mar-26 bulk SMS opt-out reversed.** Twilio voice number now answers calls with a TTS greeting and records voicemails up to 2 minutes. Recording webhook downloads the MP3 from Twilio (server-side, with Basic Auth), uploads it to a new public Supabase Storage bucket (`voicemails`), inserts a row in both the new `voicemails` table and `sms_messages` (so it shows in the inbox), and emails `info@familylaundry.com` with an inline Listen button. Transcription arrives via a second webhook hit and updates both rows. **Admin inbox UI:** new `renderMessageBubble(m)` helper detects `🎙️ Voicemail` body prefix and renders an inline `<audio controls>` element with `preload="metadata"` (so duration shows on load). Migration `session_138_voicemails_table` (mid-session counter mistake — name kept as-is). Twilio number Voice URL points at `https://umjpbuxrdydwejqtensq.supabase.co/functions/v1/twilio-voice-webhook`. Two old voicemails (recorded before the upload path was working) backfilled to Storage via one-shot `voicemail-backfill` function. Two diagnostic functions (`voicemail-diagnostic`, `voicemail-backfill`) decommissioned to return 410 + require JWT. ⚠️ **Critical learning for any future Storage code:** this project uses the new `sb_secret_*` API key format (41 chars, not a JWT). The Storage REST endpoint REQUIRES BOTH `Authorization: Bearer <key>` AND `apikey: <key>` headers — sending only `Authorization` returns "Invalid Compact JWS" because Storage tries to parse the bearer as a JWT. PostgREST already worked because all existing dbGet/dbPost helpers send both. **Bulk opt-out reversal (separate from voicemails, surfaced because David noticed his own account showed "Opted out — texts won't be delivered"):** found 2,940 customers (54% of total) flagged opted-out on 2026-03-26 in two clustered writes (01:44 UTC: 2,546 with consent; 03:11 UTC: 381 without). No real STOP messages from any of them around that time, no DB triggers touch the column. Source script never identified — most likely a one-off run tied to A2P 10DLC approval (Mar 16) or Starchup migration. David made the explicit call to fully restore (TCPA compliance risk acknowledged). 2,927 affected rows snapshotted into new `sms_optout_restore_log` table, then `sms_marketing_opt_out_at` cleared. Post-state: 21 customers still opted out (genuine STOPs from non-bulk dates, untouched). Rollback is a single UPDATE joining `customers` to `sms_optout_restore_log`. **Open question:** if the Mar-26 script was a deliberate "post-A2P reset, require re-opt-in" move, this restore re-introduced that risk — worth investigating in a future session whether a re-opt-in flow is needed. Commits `c9c6130` (inline player), `7bab2a5` (preload metadata).*

*Prior: Apr 28, 2026 — Session 137 (pt 6). **POS topbar redesign — brand swap, declutter, purple-forward.** David's feedback: the prior topbar leaned too heavily on a Square-style dark-purple square + washing-machine SVG icon, had too many separate elements (a standalone "Switch" cashier button, a verbose 2-element reader status block, a duplicate-of-the-OS-chrome clock), and the elements all had different heights so the row didn't feel pixel-aligned. **Changes (pos/index.html, ~258 net lines added, 139 removed):** (1) Brand wordmark on the far left — `<img src="../assets/logo_white_bg.png">` at 40px tall, replaces the dark-purple square. (2) Standalone "Switch" button removed; its function was folded into the cashier pill itself — click `Maria ▾` to end shift / switch cashier. (3) Clock removed — duplicates the OS clock on the iPad chrome. JS function kept as null-safe no-op so re-introduction is plug-and-play. (4) Reader-status compacted from `<div>` + `<span>` + `<button>` ("○ Not connected · Connect reader") to a single 40px `.reader-pill` with a `.pill-dot` and a label, toggled via a `.connected` class. (5) Single `.pill` base class introduced as the source of truth for all topbar pill heights — every chrome element now exactly 40px tall and shares hover/active behaviour. (6) Purple-forward palette: hover states tint everything `var(--accent-light)`, attached customer chip fills solid `var(--accent)`, queue-badge `has-items` state and `active` state both use accent. (7) Topbar height 64px (was 68px), 10px gap (was 16px). **Brand asset note:** the wordmark bakes in "Oakland CA" tagline; per-location distinction is handled by the existing site-selector pill (Main / Foothill etc.). **Earlier in session 137:** `skip_confirmation` template split into recurring vs one-time.

*Prior: Apr 28, 2026 — Session 137 (pt 5). **`skip_confirmation` template split into recurring vs one-time.** The single `skip_confirmation` template was sending the same copy ("…See you on your next pickup date.") for both recurring and one-time order skips. For one-time customers there is no next pickup queued, so the wording was misleading. Migration `session_137_split_skip_confirmation` renamed the existing template to `skip_confirmation_recurring` and inserted a new `skip_confirmation_one_time` with CTA copy ("…Text PICKUP anytime when you're ready to schedule another."). `handleSkip()` in `twilio-webhook` v33 picks the right key by checking `order.recurring_interval` (non-null = recurring, null = one-time). Hardcoded fallback messages also branched correspondingly. Both templates appear in the admin Notifications tab as separate editable rows for full admin control without code deploys. **Earlier in session 137:** PICKUP keyword fixed.

*Prior: Apr 28, 2026 — Session 137 (pt 4). **PICKUP keyword fixed (Parker Thomas case).** Customer Parker Thomas texted "Pickup" — system replied "We had trouble booking your pickup" within 1 second. Investigation found the `twilio-webhook` PICKUP handler hardcoded `source: 'sms'` in its order INSERT, but the `orders_source_check` CHECK constraint only allows `scheduled / walk_in / customer_app / recurring`. Every PICKUP attempt since deployment failed with constraint violation; the handler caught the error and sent the generic apology. **Scope:** 0 orders ever booked via SMS; 4 PICKUP messages received from 2 distinct customers, 100% failed — feature was completely broken since launch. Just hadn't been widely advertised, so the cumulative damage was small. **Two fixes shipped in `twilio-webhook` v32:** (1) `source: 'sms'` → `source: 'scheduled'` (mirrors admin-booked orders; if we ever want to distinguish SMS-booked orders specifically, add an `intake_channel` column rather than reusing source — note added in code comment). (2) Pickup/delivery windows now use a single sub-window (`window_start` → `window_start + arrival_window_hours`) instead of the full template window. The full-window version would have passed INSERT (the `enforce_window_in_route_template` trigger only fires on UPDATE) but would have shown customers a 4-hour window in their confirmation SMS instead of a real 2-hour pickup slot, and any later admin reschedule would have been rejected by the alignment trigger. Fetches `arrival_window_hours` in the route_templates SELECT so the math is template-driven not hardcoded. **System-wide assessment** per the new principle: confirmed broken since launch (0 successful PICKUP orders ever); the constraint mismatch should have been caught by a deploy-time happy-path test for each Twilio keyword. Tech debt: a synthetic PICKUP/SKIP/STOP test against staging would prevent this class. For now, added an explicit code comment near the source value pointing at `orders_source_check` so a grep for 'sms' surfaces the constraint relationship. **Earlier in session 137:** Emily Wong "thrown out" bug fixed across all 3 apps.

*Prior: Apr 28, 2026 — Session 137 (pt 3). **Emily Wong "thrown out" bug fixed across all 3 apps.** Customer reported being kicked back to login every time she tried to place an order. Trace through auth logs found `GET /verify` returning 403 "One-time token not found" twice during her session — once 32 seconds after a successful magic-link login, again 90 seconds after a successful SMS-OTP recovery login. **Root cause:** `customer-app/index.html` never strips `?token_hash=...` (or any other auth param) from the URL after Supabase JS auto-redeems it. Magic-link tokens are single-use; if the URL keeps the token, any subsequent re-init of supabase-js (tab refresh, PWA reload, browser back, multi-tab reactivation) re-attempts /verify, gets 403, and supabase-js emits SIGNED_OUT — the customer-app's auth handler then clears state and shows the login screen. **Fix:** added `_stripAuthUrlParams()` IIFE immediately after `createClient()` in all 3 apps (customer-app, driver-app, admin-dashboard for parity). Strips `token_hash`, `type`, `code`, `error`, `error_code`, `error_description`, `access_token`, `refresh_token`, `expires_in`, `expires_at`, `token_type` from query string; clears auth-related hash fragments. Idempotent — no-op when params absent. **Why this hits more customers now:** session 110's `send-magic-link` v17 + this morning's orphan-auth cleanup mean more customers will successfully take the email-login path going forward, so this bug would have escalated without the fix. **Earlier in session 137:** POS launderer assignment shipped.

*Prior: Apr 28, 2026 — Session 137 (pt 2). **POS launderer assignment shipped.** Inserted a required "Assign Launderer" step between Intake and Mark Ready in the POS walk-in flow — fixes the silent gap where every walk-in's bags + weight were attributed to whoever happened to advance the status, not the actual launderer. **Migration `session_137_assign_order_launderers_rpc`:** 2 new SECURITY DEFINER RPCs — `get_active_launderers()` (read function so POS can populate the picker without admin RLS on `launderers`) and `assign_order_launderers(p_order_id, p_assignments JSONB, p_actor_name)` (atomic write of single OR split assignment, 1-3 launderers, validates everything, replace-semantics on `order_folding_assignments` only after validators pass — the session 133 lesson). Both granted to `authenticated` only, anon explicitly REVOKEd. **POS UI:** new yellow "⚠️ Assign launderer" warning chip on processing cards, opens a modal with 1/2/3-way split tabs mirroring admin's foldSplit UX. Mark Ready button hidden until assignment exists; `advanceQueueOrder()` re-checks before transitioning (belt-and-suspenders). 5 RPC validation tests pass: empty array, invalid launderer_id, duplicates in split, nonexistent order, happy path. **POS test data cleanup:** hard-deleted the 7 historical `source='walk_in'` test orders + 11 dependent order_events (all snapshotted to `_archive._pos_test_orders_*_20260428` for full reversibility). 5 of the 7 had real Stripe charges totaling $4.91 — left in Stripe as historical record per David's call. **New audit Check 20 added** — flags any walk-in order in `ready_for_delivery`/`delivered` with no launderer assigned (catches anything that ever bypasses both UI gate + advanceQueueOrder guard). Skill rebuilt + presented for reinstall. **Earlier in session 137:** Morning rounds + targeted fixes. **Bug fixed: billing_status overwrite (the Dana Ross / Craig Griffin case).** Audit Check 3b flagged 2 delivered orders with `billing_status=NULL` but valid Stripe payment intents + `billed_at` stamped. Investigation traced to admin-dashboard `opChargeOrder()` (the Billing-tab "Charge $X" button): after `charge-order` correctly set `billing_status='paid'`, a follow-up line `db.from('orders').update({ billing_status: null })` clobbered it back to NULL. The previous RPC-refactor audit had explicitly cleared this line as "standalone and fine — single field cleanup after an already-successful Stripe call" — that judgment was wrong because the comment didn't notice charge-order had already written 'paid'. **Blast-radius check** before fix: scanned every `charge-order` callsite across all 3 apps, every `billing_status` write, and every DB function referencing 'charged' — pattern confined to one file. **Reconciliation:** verified both Stripe payment intents Succeeded (David checked dashboard), then UPDATE'd `billing_status='paid'` on Dana #3045 ($228.95) and Craig #3260 ($81.95). **Other audit fixes:** deleted 2 orphan auth users (Jessica Rodriguez, Dorothy Monza) — both were shadowing real customer email accounts, blocking magic-link sign-in; analyzed 6 silent-reschedule false-positives — all triggered by the Apr 15 sub-window correction sweep, not customer-visible time changes (no SMS needed). **Stripe customer ID for Heather Covyknight:** found that 50 customers (42 pre-migration) have a `stripe_customer_id` but no `card_last4` saved — natural "started Add Card flow but didn't finish" abandonment, not a migration data carryover (David confirmed no Stripe IDs were imported; every ID was created by WashRoute's own setup-checkout flow). David is calling Heather directly to take her card over the phone. **November 2025 Laundry Day subscriber retention analysis:** 188 subscribers from the November export → 84% retention (158 placed orders Apr 1–28 in WashRoute), 16% churn (30 customers, ~$78K total LTV concentrated in 9 high-value names). Tight Oct 21–27 cluster of 11 churns predates WashRoute migration; 5 March-2026 churns line up with the platform switch. **New guiding principle added:** "Every bug found triggers a system-wide assessment" — the "is this widespread?" + "how do we prevent recurrence?" questions are now first-class steps after every fix. **Pending from this session:** 7 genuinely charge-failed orders awaiting customer outreach; Drew School #3063 (picked up Apr 24, never delivered — needs investigation); Check 18 enhancement to filter sweep-artifact false positives. Last commit pending — committing this entry now.*
...
**Trigger:** While placing a test order on the Commercial pricelist (David Macquart customer record), David noticed the final Confirm screen showed `$16.70` total when it should have been `$58.70`. Bag Count step correctly showed `$53.70` ($43.75 base + $9.95 delivery, no tip yet); Confirm step regressed to `$16.70` ($1.75 × 1 bag + $9.95 + $5 tip). The bug only affects services with `pricing_type='per_lb'` — i.e., the Commercial pricelist.

**Root cause:** Two sites computed `base_price * bags` without checking `pricing_type`:
1. `customer-app/index.html` `buildConfirmSummary()` — visible total + breakdown line on the final review screen.
2. `admin-dashboard/index.html` `saveOrder()` — the manual "Add Order" modal. Saved `total_amount` and the `line_items[base]` row directly to the DB.

**Blast-radius check (washroute-qa skill):** searched all 3 apps for `basePrice * bags`, `base_price * bags`, and "price × bags" patterns. Confirmed every other pricing site already correctly branches on `isPerLb`:
- customer-app `updateEstimate` (bag count step) — ✅
- customer-app `placeOrder` (actual order INSERT) — ✅
- customer-app `saveEditOrder` (order editing) — ✅
- customer-app receipt modal — ✅
- admin `opSaveDetails` + `opUpdateCostBreakdown` (admin order edit) — ✅
- driver app — doesn't touch pricing at all
- DB triggers/functions referencing `pricing_type` — none compute totals from it; affected logic is purely UI.

**Customer impact assessment:**
- **Customer app:** Commercial customers placing orders saw a misleadingly low total on Confirm but the actual saved `total_amount` was correct (since `placeOrder` already gates on `isPerLb`). So no customer was actually billed wrong via the customer-app path — they were just confused on the review screen.
- **Admin Add Order modal:** any Commercial customer order created via this path WAS saved with a wrong, way-too-low `total_amount`. Worth a quick query to see if any Commercial customers have orders created via admin in the last few weeks with implausibly low totals — David hasn't created Commercial orders this way recently to his recollection, so probably zero impact, but worth confirming when convenient.

**Fix:** mirrors the canonical per-lb math used by `updateEstimate`/`placeOrder`/`saveEditOrder`:
```js
const isPerLb = svc?.pricing_type === 'per_lb';
const estLbs  = isPerLb ? bags * (parseFloat(svc?.lbs_per_bag) || 25) : 0;
const baseTotal = isPerLb ? (estLbs * basePrice) : (bags * basePrice);
```
Plus the visible line label updated to show `Est. 25 lbs × $1.75/lb` instead of `× 1 bag` for per-lb services.

**Tech debt logged (non-blocking, hygiene):**
1. The per-lb math is now duplicated across 7 sites in the codebase (5 customer-app + 2 admin). Pulling it into a single helper — e.g., `getOrderPricing(svc, bags, prefs, fees, opts)` returning `{baseTotal, addonsTotal, total, baseLabel, lineItems}` — would make this class of bug structurally impossible to reintroduce. Worth tackling as a UI-layer cleanup; could ride alongside the next RPC refactor pass.
2. `services.pricing_type` allows NULL today. Worth a small migration: `ALTER TABLE services ALTER COLUMN pricing_type SET NOT NULL, SET DEFAULT 'per_bag';` so an admin can never save a service with a null type that would silently fall through to per-bag math everywhere.

Commit `3f271d9`. No DB migrations. No edge function changes.
