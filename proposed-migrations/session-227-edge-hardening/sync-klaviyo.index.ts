...
Deno.serve(async (req) => {
  try {
    ...
    if (!SYNC_SECRET) {
      console.error('sync-klaviyo: KLAVIYO_SYNC_SECRET is not set — refusing all calls')
      return new Response(JSON.stringify({ error: 'forbidden' }), { status: 403 })
    }
    if (!secretMatches(body.secret)) return new Response(JSON.stringify({ error: 'forbidden' }), { status: 403 })
