...
| C12 | **Rapid charges TWO different fees, and only one of them is interest.** | Read straight off the lender's own fee table in the portal, 2026-08-19 (43 entries, 11/2025–08/2026). **Balance Fee** — 41 entries, $28,901.62, weekly, 0.894% of the outstanding balance: this is the interest of C11, and the portal's 08/17 entry of **$485.49** matches the figure predicted from the balance ratio to the cent. **Draw Fee** — 2 entries, $4,480.00, **exactly 4.00% of each draw** ($4,000.00 on the $100,000 initial draw 2025-11-03; $480.00 on the $12,000 draw 2026-03-12). The Draw Fee is a genuine fixed one-time origination charge — the thing C11 proved the *weekly* fee is not. Under ASC 835-30 that is a debt issuance cost (capitalise against carrying amount, amortise to interest expense over the term), not interest as incurred. **CORRECTION, same session:** an earlier note here said neither draw fee existed in `loan_splits`. That was wrong — it searched for exact amounts of 4000.00 / 480.00 and found none because both are **bundled inside larger rows**: the $4,000 sits in the 2025-12-31 lump journal ($7,029.84 of 2025 balance fees + $4,000.00 = $11,029.84 exactly) and the $480 sits in the 2026-03-31 split ($742.60 balance fee + $480.00 = $1,222.60 exactly). Both are therefore **already expensed as interest**, in FY2025 and FY2026 respectively. Full reconciliation against the lender's 43-entry fee table balances to **$0.00** (see C14). So the CPA question is *reclassification of an existing entry*, one of them in a filed year — not recording a missing one. |
| C14 | **Our Rapid interest reconciles to the lender's own fee table exactly — to $0.00.** | Verified 2026-08-19 against all 43 entries read from the portal (which themselves total $33,381.62, matching the statement). Rapid's 41 balance fees total $28,901.62; less the 2026-08-17 fee of $485.49 that has not been ingested yet gives $28,416.13; plus both draw fees ($4,480.00) gives **$32,896.13 — exactly what `loan_splits` holds.** 31 of our 32 per-period amounts match a lender fee to the cent; the one that doesn't (2026-03-31, $1,222.60) is the bundled row above. **The only genuinely missing fee on this loan is 2026-08-17, $485.49.** This is the strongest evidence yet that the ingestion logic is correct — and it means the tie-out's $1,056.19 Xero-vs-lender gap is NOT a missing-fee problem. |
| C13 | **Rapid's statement balance is an AMOUNT-OWED measure that includes charged fees — our 8 anchors are mislabelled `principal_only`.** | Rapid's own tooltip, verified in the portal 2026-08-19: *"The total amount owed, including all unpaid draw amounts, **fees and charges** on the account as of today's date."* The statement roll-forward is built the same way and reconciles exactly: `100,000.00 opening + 12,000.00 draws + 33,381.62 fees − 92,712.27 received = 52,669.35 closing`. **Fees are ADDED INTO the balance**, so this is not a principal-only figure. **However the comparison against Xero is still like-for-like** — our reclass journals credit each fee back to the loan account, so Xero's Rapid balance is built on the same amount-owed basis. **The $1,056.19 tie-out exception is therefore a real gap, not a basis artifact.** Do NOT "fix" this by re-typing the anchors to `total_payback`: that would make `computeTieOut` refuse them and silently convert a true exception into `not_comparable`, trading a real signal for false silence. See the latent inconsistency in PROJECT-NOTES item 15 before changing anything. |
