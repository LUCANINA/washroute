...
- **Discount on subscription overage (294d, applied):** `apply_subscription_usage_fn` now nets the order's % discount off the overage it adds and rebuilds the discount line. Tested (rolled back): #14685 shape → $37.40 (was $44), stable on re-fire. Past cases: #14685 $6.60, #12228 $3.71 still owed back.
- **Dead subscription on recurring orders (294d, applied):** recurring generator only carries active/past_due/paused subscriptions and re-prices from the current pricelist. Tested (rolled back): Candy's next order → 2 bags × $65 + $9.95 = $139.95. #14853 (9/19) repriced from $0 to $139.95. She has skipped 9 weeks straight; with the link gone, the 2-skip auto-stop now applies to her chain.
- **Stuck phone sign-ins (294e/294f, applied + prepare-phone-otp v25 deployed):** root cause was a missing grant — every login deletion failed ("permission denied for table profiles"), so stale phone logins were never cleared; also the lookup only read the first 1000 customers / logins. All 6 stuck customers relinked. ~85 other stale phone logins remain; each clears itself the next time that customer requests a code.
