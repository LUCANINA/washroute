...
**2. Bentley Upper School duplicate merged.** Two customer records sharing phone `(510) 289-3067` and email `jludwig@bentleyschool.org`: canonical record (ee83…, "Bentley Upper School (Joanne Ludwig)", since Oct 2024, 7 orders, on_account billing, profile linked) vs duplicate (8c63…, "Bentley Upper School", created May 4 2026, 0 orders, no profile, just 1 duplicate "Home" address at the same physical location as canonical's "Work" address). Trivial merge — snapshot both records + the duplicate address to `_archive._merge_backup_bentley_20260506_*`, delete the duplicate address (same coords already covered by canonical's Work label), delete the duplicate customer. Migration `session_144_merge_bentley_duplicate`. No FK cleanup needed (newer record had nothing else attached). Canonical Bentley intact and untouched.

**3. Audit Check 12 (orphan auth users) false-positive fix.** Session 143 flagged the gap: the check was joining `auth.users LEFT JOIN customers c ON c.profile_id = au.id WHERE c.id IS NULL` — but drivers, admins, and pos_device accounts legitimately have no customer row, so they showed up as orphans every morning along with real stuck customers. Session 143's tech debt note suggested adding `AND p.role = 'customer'` to scope the check to customer-role auth users only. **Fix shipped in `washroute-audit.skill`** — added `JOIN profiles p ON p.id = au.id AND p.role = 'customer'` to Check 12. Verified against today's data: all 3 prior false positives drop out (Jose driver, Nathie driver, David admin), and 1 real candidate surfaces that was previously buried — `cmshure@gmail.com` (March 25 2026 signup, customer role, no linked customer record). Worth investigating in a future round; for now flagged for David. Skill rebuilt via the standard unzip → edit → rezip → present_files pattern. Snapshot of prior skill bytes not strictly captured, but the skill is in git and the prior version is recoverable from any pre-session-144 commit if needed.

**4. Aimee Eng order #3801 — silent $5 tip drop.** David reported Aimee was showing a "$5 balance" despite already paying. Root cause: order #3801 (May 4 delivery) was fully credited at intake — line items totaled $95.95 ($59 base + $24 weight overage + $3 Oxi + $9.95 delivery), $95.95 of credit applied via `apply_customer_credit_to_order` from her $165 balance, `total_amount` written as $0.00. But the $5 tip on the order was NEVER processed: `tip_amount = 5`, `billing_status = NULL`, `billed_at = NULL`, no tip credit_use logged. The $5 sat orphaned for 2 days, surfacing as a "balance" in the admin customer panel.

**5. Why session 128's "credits-first / card-fallback / Issues-tab-fallback" tip invariant wasn't honored.** Forensic trace on `rack_order` RPC: the function had a buggy `v_already_charged` check that treated any non-NULL `stripe_payment_intent_id` as a real charge, INCLUDING the function's own sentinel string `'credit_applied'` that the credit-close UPDATE writes alongside `billing_status='paid'`. If an order ever ended up with `spi='credit_applied'` set without `billing_status='paid'` set (Aimee's case — exact origin untraceable beyond log retention; could have been a momentary RPC error, half-applied UPDATE, manual SQL during testing, or a race with another mutation), the next call to `rack_order` would short-circuit to the `already_charged` branch and skip its own billing UPDATE entirely — leaving billing unclosed and the tip silently dropped. Cross-checked the last 6 fully-credited tip orders that ran through `rack_order` (#2972, #3079, #3442, #3829): all 4 of the post-RPC orders closed billing correctly with the $tip credit_use logged. Aimee's #3801 is the lone outlier.

**6. Manual heal for Aimee + structural fix for the rack_order bug class.** Two migrations:

   - **`session_144_heal_aimee_eng_3801_tip`** — mirrors what `rack_order`'s `fully_credited_tip_from_credit` branch should have done on May 4. Calls `apply_customer_credit_to_order(Aimee, $5, 'Tip for order #3801 (manual heal — rack_order skipped tip branch May 4)', #3801, 'credit')` to atomically deduct $5 from her $69.05 balance + log the customer_transactions row, then UPDATEs the order with `billing_status='paid'`, `billing_payment_method='credit'`, `billing_notes='Fully covered by credits (incl. $5 tip) — manual heal session 144'`, `billed_at=NOW()`. Plus an `order_events` row noting the heal. Pre-fix order state snapshotted to `_archive._heal_aimee_eng_3801_20260506`. Aimee's credit balance is now $64.05; her account no longer shows the "$5 balance".
   - **`session_144_rack_order_strict_already_charged`** — tightens `v_already_charged` to:
     ```
     v_already_charged := (v_order.billing_status = 'paid'
                           OR (v_order.stripe_payment_intent_id IS NOT NULL
                               AND v_order.stripe_payment_intent_id != 'credit_applied'));
     ```
     New logic: treat as already-charged iff `billing_status='paid'` OR `spi` is a real (non-sentinel) Stripe PI. Defense-checked against all 5 scenarios — sentinel + NULL (Aimee) re-evaluates and heals; real PI + NULL (Check 3b bug class) still treated as already-charged (no double-charge); real PI + paid still already-charged; sentinel + paid still already-charged via billing_status; spi NULL still NOT already-charged (normal first rack). Self-healing — any future order in the same half-state will close billing correctly on rack. Prior `rack_order` definition snapshotted to `_archive._backup_function_defs` (5708 bytes) before replacing.

**7. Sweep for sibling orders.** SQL across all orders for `total_amount=0 AND tip_amount>0 AND billing_status IS NULL AND status IN (processing/folding/ready_for_delivery/out_for_delivery/delivered) AND source != 'walk_in'` returned 2 rows: Aimee #3801 (genuine bug, fixed above) and Sarah Bseiso #3870 (created May 4, status=folding — just in-progress, hasn't been racked yet). Sarah's order will close cleanly on the next rack via the patched `rack_order` (she has $414.25 in credits → tip-from-credit branch). Not a bug.

**Tech debt left:**
- `cmshure@gmail.com` orphan auth user surfaced by patched Check 12 — needs investigation. Could be a typo signup or a real stuck customer worth reaching out to.
- The 11 unpaid delivered orders flagged in the morning audit ($1,636 total, no card on file) — same class as session 83's recovery sweep; deferred for a focused outreach session.
- Pre-rack origin of Aimee's `spi='credit_applied'` half-state remains unknown. The `rack_order` patch makes it self-healing going forward; whatever set the sentinel pre-rack is now harmless. If the same anomaly recurs we'll see it in `order_events` because the heal path now logs an event.

**Commits this session:** `[next]` (PROJECT-NOTES + washroute-audit.skill update). DB writes: data fixes (Bentley merge, Aimee tip heal) + 3 migrations (`session_144_merge_bentley_duplicate`, `session_144_heal_aimee_eng_3801_tip`, `session_144_rack_order_strict_already_charged`). No edge function deploys, no new cron jobs, no new tables.*

---

*Prior: May 5, 2026 — Session 143. **Cindy Downing email mystery → broader email-mismatch sweep + root-cause patch.** Investigation, data fixes across DB + Stripe, RPC migration to prevent recurrence.*
...
**7. Morning rounds audit — 21 checks run.** Most clear; surfaced 1 P0 + several P1s. Resolved this session:
- **Elizabeth Gettinger #3763** — unrouted, surfaced with `routing_error: "No matching delivery route with capacity for this time window"`. Investigation: pickup was already routed cleanly; the *delivery* window was 6–9 PM but Hayward PM template starts at 7 PM, no template matched. Force-routed via `reschedule_order_leg` RPC with auto-snap to Hayward PM (May 9 7-10 PM, route `0e82a876…`). `routing_error` cleared, delivery_run_id set, route_stop created.
- **3 orphan auth users deleted:** Sukhesh Miryala (orphan email-auth shadowing real customer), AlbertH HartIII (stuck phone-OTP — 14 orders), Sheila McCune (stuck phone-OTP — 0 orders). Snapshotted to `_archive._orphan_auth_delete_20260505` (full row + identities). All three should now sign in cleanly to their real accounts on next attempt.

**8. Root-cause: recurring chain drift after one-off cross-zone reschedules — fixed (migration `session_143_recurring_snap_to_template`).** Trace on Elizabeth's prior order #3242: a driver legitimately rescheduled her delivery from Hayward PM → Berkeley PM → Alameda PM (settled at 6-9 PM Alameda) on May 3 as a one-off accommodation. The recurring trigger then projected the 6 PM window forward into next week's order #3763, which the customer's home zone (Hayward) couldn't match — Hayward has no 6 PM template. Bug: `trg_create_recurring_order_fn` was carrying forward the previous order's exact window without verifying it was valid in the customer's home zone. **Fix:** new helper `snap_window_to_template(p_zone_id, p_target_ts, p_override_template_id)` returns the closest valid sub-window for a zone+day, and the recurring trigger now calls it for both pickup and delivery before INSERT. Self-heals after any one-off cross-zone reschedule. Helper falls back to projection (current behavior) if no active template matches the zone+day. Verified end-to-end: 6 PM PT → Hayward PM 7-10 PM, 8:30 AM PT → Hayward AM 7-10 AM. Prior trigger snapshotted to `_archive._backup_function_defs`. anon explicitly REVOKEd from helper; authenticated + service_role granted.

**Tech debt left from morning rounds (not addressed today):**
- 🔴 **SF route May 5 (today) hard-blocked at 21 stops** (limit 15, ceiling 18). Active issue. Would prevent any new SF stop from being routed today.
- ⚠️ Bentley Upper School duplicate customer (same phone + email) — clean merge candidate.
- ⚠️ Carol Stevenson #3399 + #3750 — two scheduled orders for May 7. Possibly intentional, possibly accidental — needs eyes.
- ⚠️ Jennifer Lamphere #3078 — `delivery_failed` but pickup stop still `pending`. Stop/order desync.
- ⚠️ Order #3556 walk-in Apr 30 with no launderer assignment (operational gap from before session 137's POS gate; backfill if you can identify who washed it).
- 📋 Audit Check 12 false-positives (drivers + David's admin auth). Worth extending the WHERE clause to `AND p.role = 'customer'` so future audits don't re-flag staff/drivers.
- 📋 11 unpaid delivered orders with no card on file (Carol Stevenson, Irene Atkins x2, Meg Lamberton, Ruth Cossey x2, etc.) — same outreach class as session 83's recovery sweep. `outreach-sms.js` / `outreach-email.js` handle these.
- 📋 4 zone-overlap pairs (largest 0.87 sq km Hayward/Concord). Same class as session 133's Berkeley fix; minor priority.

**Commits this session:** `d0bf4b1` (PROJECT-NOTES session 143 entry), `9bb5f6e` (Customers tab default-filter fix), `[next]` (this notes update). DB writes: data fixes + migrations `session_143_claim_existing_customer_starchup_placeholder` + `session_143_recurring_snap_to_template`. 3 orphan auth user deletions. Stripe API writes for Cindy + Ereene. Edge function deploys: `sync-stripe-customer-email` v1→v2→v3 (v3 = 410-Gone stub). Force-route via `reschedule_order_leg` for #3763.*
...
**2. Admin Discounts page now has a Date Created column.** Mid-session conflict here: David's session-141 work had already added `Redeemed` + `Last Use` columns to the same page using a `_redemptionCells(d)` helper. Rebased onto that, manually merged the column-header conflict, and slotted `Created` between `Code` and `Redeemed`. Final layout: `CODE | CREATED | REDEEMED | LAST USE | [Archive]`. No DB query change — `loadDiscounts()` already does `select('*')` so `created_at` was in the cache. Newly migrated coupons all show today's date (May 4) since insertion stamped a fresh `created_at`; the original Starchup creation dates aren't preserved.

**3. POS goes fullscreen at every viewport — kill the mockup bezel.** `pos/index.html` had a `.device` + `.device-screen` wrapper from very early in the project that drew a black-rounded "iPad in landscape" frame at 1180px max-width. Built originally as a desktop preview mockup, it kept drawing on actual iPads — wasting ~30-40px on every edge plus the rounded-corner radius INSIDE the iPad's real bezel. **First attempt** kept the mockup look at desktop sizes (≥1367px) for design preview; David tested in Chrome and reported "no change" because Chrome on his Mac is wider than that. **Second attempt** dropped the desktop exception entirely — the mockup served its purpose, and POS is now used from desktop browsers too (management/QA), so the framed look is just clutter. Both `.device` + `.device-screen` collapse to 100vw × 100vh unconditionally. Also removed dead helper classes (`.page`, `.page-header`, `.page-title`, `.page-sub`, `.screen-label`) that were only referenced by the retired chrome.

**4. POS sales tax label was hardcoded at 9.25%.** The cart's Tax row showed `<span data-i18n>Tax (9.25%)</span>` as static HTML — the label lied even though the underlying math has been correct since session 140 (`TAX_RATE` is loaded from `settings.sales_tax_rate`, currently 0.1075). Receipts printed correctly at 10.75%, the Charge button charged correctly, only the on-screen label was wrong. Replaced the static span with `<span id="taxLabel">` populated by a new `refreshTaxLabel()` helper called from (a) `loadTaxRate()` after the DB load completes, and (b) `applyTranslations()` so EN/ES toggle reformats correctly. Spanish translation simplified from `'Tax (9.25%)' → 'Impuesto (9.25%)'` to just `'Tax' → 'Impuesto'` (the percentage now formats in JS). Receipt printing code (Star Markup + HTML fallback) was already dynamic via `(TAX_RATE * 100).toFixed(2)` — no change needed there.

**5. POS receipts reworked as bag tags for the laundering team.** David's spec: the laundering team uses the printed receipt as a bag tag to identify whose laundry is whose in the back-of-house. Three changes:
- **Customer name front and center.** Star Markup ticket now leads with the customer name at size 2:2 (double-height + double-width) bold uppercase centered — the largest thing on the page. Below: bold "Order #N" at size 1:2, then timestamp. The "FAMILY LAUNDRY / website / Thank you!" header demoted to the footer where it's still legible but doesn't compete with the name. Browser-fallback HTML receipt mirrors the same layout (22px bold uppercase centered name).
- **Default 2 copies per sale.** `queuePosPrintJob(order, copies = 2)` now takes an optional copies param defaulting to 2 (capped at 5). Initial Print on the success modal queues two identical pending `print_jobs` rows — CloudPRNT picks them up sequentially, prints + cuts each independently. Browser-fallback popup repeats the body with `<div page-break-before>` separators so the print dialog generates matching multi-copy output.
- **Reprint from the Retail Queue.** Tapping a queue card (existing 3-step delete UX) now reveals two side-by-side actions instead of just Delete: `🖨 Reprint receipt` (gray, 1 copy) and `✕ Delete order` (red). New `reprintQueueOrder(orderId)` helper looks up the order in `queueOrders` and calls `queuePosPrintJob(order, 1)`. **Refactor along the way:** both receipt builders (`buildPosReceiptMarkup` + `buildPosReceiptHtml`) now prefer `order._customerName` (set by `loadWalkInQueue` on every queue row) over `currentCustomer?.name`. Means reprints work cleanly without temporarily swapping the active-cart globals — same code path serves both initial-sale and queue-reprint sources.

**Tech debt left for later:**
- Reprints can only fire from the same physical iPad whose `printer_token` is bonded to the register that originally took the sale. If a different register tries to reprint, the print_job goes to the wrong printer (or doesn't print at all if no token is set on the alternate register). Cross-register reprint would need either (a) a printer-selector UI, or (b) routing reprints through the original register's token regardless of which device is initiating. Not pressing today since each location has one register.
- The `currentCustomer` swap pattern was removed from `reprintQueueOrder` but the customer-app and admin still have similar swap-globals patterns in places. Worth a future audit when the RPC refactor work continues.

**Mid-session git incident:** Hit a stale lock chain in `.git/` — `refs/remotes/origin/main.lock.s140p1d.stale` and `HEAD.lock` both blocked any pull/rebase. Sandbox initially refused to delete (Operation not permitted); used `allow_cowork_file_delete` to enable, then cleaned them. Then `git pull --rebase` failed mid-flight on the discount-page conflict above; rebase aborted left the index in a half-staged state, recovered with `git reset --hard HEAD` + retry.

**Commits:** `fef8870` (Date Created column on Discounts), `2e772b8` (POS fullscreen on iPad ≤ 1366px), `3f7c98a` (POS fullscreen at all viewports — superseded above), `9698def` (tax label fix), `b4e0859` (receipt rework + 2-copy default + queue reprint), `[next]` (this notes update). DB inserts: 24 rows in `discounts`. No new migrations, edge functions, or cron jobs.*
