-- Session 168b — recurring SUBSCRIPTION cycles carry forward ALL customer-chosen
-- extras (paid add-ons like Vinegar/Oxi, AND the same-day surcharge), not just
-- the same-day surcharge.
--
-- Context: the Subscription branch of trg_create_recurring_order_fn rebuilds
-- line_items to [base $0, delivery_fee $0] (both absorbed by the plan). Session
-- 168 added carry-forward for same_day_surcharge only. With standing paid extras
-- now selectable at subscription setup (Vinegar/Oxi etc., stored as `addon` line
-- items), those must also recur + bill every cycle. Generalize: preserve every
-- parent line item whose type is NOT base/delivery_fee, adding each to total.
-- charge-order bills total_amount regardless of subscription, so the card is
-- charged for the extras each week. The laundry team reads the `addon` line items
-- to know which preferences to apply.
--
-- Reversibility: re-apply session_168 (same_day_surcharge-only carry block).
...
BEGIN
  ...
  ELSIF v_pricelist = 'Subscription' THEN
    ...
    -- Session 168b: carry forward ALL customer-chosen extras (paid add-ons,
    -- same-day surcharge) from the parent — anything that isn't base/delivery,
    -- which the plan absorbs. Standing extras (e.g. Vinegar/Oxi) recur + bill
    -- each cycle; the same-day delivery window itself is already preserved via
    -- the pickup->delivery turnaround interval above. charge-order bills
    -- total_amount regardless of subscription.
    FOR v_extra_item IN
      SELECT elem
      FROM jsonb_array_elements(COALESCE(NEW.line_items, '[]'::jsonb)) AS t(elem)
      WHERE elem->>'type' NOT IN ('base', 'delivery_fee')
    LOOP
      v_line_items   := v_line_items || jsonb_build_array(v_extra_item);
      v_total_amount := v_total_amount + COALESCE((v_extra_item->>'amount')::numeric, 0);
    END LOOP;
