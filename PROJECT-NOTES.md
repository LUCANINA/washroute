...
**⚠️ EDGE FUNCTIONS WITH `verify_jwt: true` NEED THE LOGGED-IN USER'S SESSION TOKEN — *not* the raw anon JWT (session 129).** When the Supabase gateway sees `verify_jwt: true` on an edge function, it rejects the request with HTTP 401 *before* the function code runs unless `Authorization: Bearer <JWT>` is a valid **user-level** JWT. The legacy anon JWT (`SUPA_KEY` / `SUPA_ANON_KEY`) is **not** accepted by this gateway check anymore — sending it as Bearer returns 401 despite being a "valid" JWT by signature. The only two call sites in the whole app that currently work against `verify_jwt: true` functions are ones that do `const { data: { session } } = await db.auth.getSession(); ... Authorization: \`Bearer ${session.access_token}\``. All 4 optimize-route call sites and the `get-stripe-fees` call at admin-dashboard line ~23609 now follow this pattern. If you add a new edge function and leave `verify_jwt: true` (Supabase's default), the client must: (1) await `db.auth.getSession()`, (2) send `Authorization: Bearer ${session?.access_token || SUPA_KEY}` (anon fallback for unauthenticated flows, though the gateway will still 401 those — so only do this for admin/driver/customer paths where a user session is guaranteed). Also keep the `apikey` header — it's still needed for rate limiting. Consider a `callEdgeFn(slug, body)` helper to make this structurally impossible to omit. The first fix (just adding `Authorization: Bearer ${SUPA_ANON_KEY}`) looked right at first — headers verified, blast radius clean — but the Supabase edge logs showed the function still returned 401 with execution_time_ms ~50ms (gateway rejection, not function code). Root cause: the gateway now rejects the anon JWT specifically on `verify_jwt: true`. Always check the actual Supabase logs (`get_logs service=edge-function`) after deploying an edge-function fix; "looks-right-in-diff" is not verification.
